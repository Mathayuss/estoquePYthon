...
from app.controllers import AssetController, CatalogController, LocationController
from app.qr_utils import generate_asset_qr_png
from app.paths import get_labels_dir

class AssetRegisterPage(ctk.CTkFrame):
    """Cadastro/CRUD de Equipamentos (Ativos) - somente Admin."""
    ...
    def __init__(self, master):
        ...
        ctk.CTkLabel(title, text="Cadastro de Equipamentos", font=ctk.CTkFont(size=16, weight="bold")).pack(anchor="w", padx=12, pady=(12, 2))
        ctk.CTkLabel(
            title,
        ...
        cols = ("id", "asset_tag", "product", "category", "location", "status", "serial_number", "qr_code")
        ...
        self.tree.heading("location", text="Unidade")
...
class AssetForm(ctk.CTkToplevel):
    def __init__(self, master, title: str, on_save, mode: str, asset: dict | None = None):
        ...
        # Unidade (local)
        ctk.CTkLabel(frm, text="Unidade (Local)").grid(row=0, column=2, sticky="w", padx=10, pady=(10, 4))
        self.var_location = ctk.StringVar(value=self._default_location_name())
        loc_values = [l["name"] for l in self.locations] if self.locations else ["Geral"]
        self.cbo_location = ctk.CTkOptionMenu(frm, variable=self.var_location, values=loc_values)
        self.cbo_location.grid(row=1, column=2, sticky="ew", padx=10, pady=(0, 10))
    ...
    def _save(self):
        try:
            ...
            if self.mode == "new":
                created = AssetController.create_asset(pid, tag, serial, notes, location_id=lid)
                self.on_save()
            ...
            if self.mode == "bulk":
                ...
                created = AssetController.create_assets_bulk(pid, qty, notes=notes, location_id=lid)
                self.on_save()
                if created:
