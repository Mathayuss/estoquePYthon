...
from sqlalchemy import create_engine, select, func, cast, Integer, event
from sqlalchemy.orm import sessionmaker
from sqlalchemy.engine import Engine

from app.models import Base, User, ExitReason, AppSetting, AssetUnit, Location
...
def _ensure_sqlite_columns() -> None:
    """Migração leve para SQLite (adiciona colunas novas sem Alembic)."""
    import sqlite3
    conn = sqlite3.connect(str(DB_PATH))
    try:
        cur = conn.execute("PRAGMA table_info(asset_units)")
        cols = [r[1] for r in cur.fetchall()]
        if "location_id" not in cols:
            conn.execute("ALTER TABLE asset_units ADD COLUMN location_id INTEGER")
        conn.commit()
    finally:
        conn.close()
