...
class CatalogController:
    ...
    def delete_category(category_id: int) -> None:
        with session_scope() as s:
            ...
            in_use = s.execute(select(Product.id).where(Product.category_id == int(category_id)).limit(1)).first()
            if in_use:
    ...
    def delete_supplier(supplier_id: int) -> None:
        with session_scope() as s:
            ...
            in_use = s.execute(select(Product.id).where(Product.supplier_id == int(supplier_id)).limit(1)).first()
            if in_use:
                raise ValueError("Não é possível excluir: existem produtos vinculados a este fornecedor.")
    ...
    def delete_product(product_id: int) -> None:
        with session_scope() as s:
            ...
            in_use = s.execute(select(AssetUnit.id).where(AssetUnit.product_id == int(product_id)).limit(1)).first()
            if in_use:
                raise ValueError("Não é possível excluir: existem equipamentos cadastrados para este modelo.")
